...
globals [cost Plist Elist PEtable no-ticks no-sec time-traveled flagcounter]
  ...
  [
    ...
    [
      ...
      [
        ...
        [
          if flagcounter > 1 and no-ticks = 0
          [
            print (word "elapsed time: " no-sec)
            print (word "total travel time: " time-traveled)
            if count evaders > 0
            [user-message (word "Evaders cannot be caught because it is isolated")]; kelangan pa ito iedit, pag hindi naman nag iinsert o nagddelete ng edge, kasi wala na talagang probability na mahuli ung evader na isolated so dapat pag gnun maidentify na kelangan na istop agad ang pursuit
            stop
          ]
...
to pursuit-strategy
  ...
  [
    ...
    ifelse no-ticks > 0
    [
       while [ pcounter != no-of-pursuers] ;; num of pursuers given by the user from interface
       [
         set pursuerpath item 1 item pcounter PEtable ;path to pursue
         set mlength length pursuerpath
         ifelse mlength > 0
         [
           set targetnode item 0 pursuerpath
           set pursuerpath remove-item 0 pursuerpath
           set oldlist item pcounter PEtable
           set PEtable replace-item pcounter PEtable replace-item 1 oldlist pursuerpath ;;continue to access the right index of what to be replaced
           move-to-node item pcounter Plist targetnode pcounter
           set no-ticks no-ticks - 1
         ]
         [
           let currentp item pcounter Plist
           ifelse is-disconnected = false
           [
             ;set PEtable []
             findpath pcounter currentp Elist
           ]
           [
             ;if initially connected ang graph
             let clusters nw:weak-component-clusters
             print(word "DISCONNECTED GRAPH!!!!")
             ;; loop through the clusters and find if pursuer is member of the cluster
             (foreach clusters [
               let cluster ?1
               ...
               [
                 ...
                 if member? currentp p-list
                 [
                   if num-evaders > 0 [
                     findpath pcounter currentp e-list
                   ]
                   if num-evaders = 0 and num-pursuers > 0 ; if there are pursuers in isolated graph but no evaders
                   [
                       let templist []
                       set PEtable replace-item  pcounter PEtable (list currentp templist)
    ...
    [
      ...
      print (word "flagcounter: " flagcounter)
    ]
  ]
  printPlist
end
...
to evader-strategy
  let ecounter 0
  let eturtle 999
  let numeva no-of-evaders
  while [ ecounter < count evaders]  ;; num of evaders given by the user from interface
  [
    ;if ecounter < count evaders [
      set eturtle item ecounter Elist
      ask turtle eturtle
      [
        if count link-neighbors > 0 [
          let etarget [who] of one-of link-neighbors
          printElist
          move-to-node eturtle etarget ecounter
        ]
      ]
    ;]
...
to initialize-pursuers
  set-default-shape pursuers "pentagon"
...
to findpath [pursuerindex curp evalist]
let ecounter 0
let costlist []
let templist []
let minvalue 99999
let currente 99999
let minindex 999
let evacount length evalist
show word "FINDING NEW PATH Pursuer: " pursuerindex

      while [ecounter != evacount ]
      [
        set currente item ecounter evalist
        set costlist lput dijkstra curp currente costlist
        set ecounter ecounter + 1
      ]
      ...
      set PEtable replace-item pursuerindex PEtable (list minvalue templist)
      set no-ticks sum map length map last PEtable
      print(word "UPDATED pursuit evader table " PEtable );
    ...
    (foreach clusters [
      ...
      [
        ...
        [
          ...
          findpath pindex currentp e-list
          set pcounter pcounter + 1
        ]
        ]
        if num-evaders = 0 and num-pursuers > 0 ; if there are pursuers in isolated graph but no evaders
